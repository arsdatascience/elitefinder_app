...
return (
    <div className="bg-white border sm:border-l border-gray-200 h-full flex flex-col bg-slate-50">
        {/* Header */}
        <div className="p-4 border-b border-gray-200 bg-white shadow-sm space-y-3">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
                <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all flex items-center justify-center gap-1 ${selectedProvider === 'openai' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    <Sparkles className="w-3 h-3" /> GPT-4o
                </button>
                <button
                    onClick={() => setSelectedProvider('anthropic')}
                    className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all flex items-center justify-center gap-1 ${selectedProvider === 'anthropic' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    <Brain className="w-3 h-3" /> Claude
                </button>
                {/* Gemini disabled for now as it focuses on Reports, or enable if wanted
                    <button 
                         onClick={() => setSelectedProvider('gemini')}
                         className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all ${selectedProvider === 'gemini' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Gemini
                    </button>
                    */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {!result && !analyzing && (
                <div className="text-center py-10 opacity-60">
                    ...
                    <p className="text-sm text-gray-500 font-medium">Estou pronto para auditar este atendimento com {selectedProvider === 'openai' ? 'GPT-4o' : selectedProvider === 'anthropic' ? 'Claude 3.5' : 'Gemini'}.</p>
                    <button
                        onClick={handleAnalyze}
